...
export async function requireActiveProfile(ctx: AuthCtx) {
  ...
  if (!company || company.status !== 'ACTIVE') {
    throw new ConvexError('Tu empresa no está disponible.');
  }

  return { userId, user, profile, company };
}

export async function requireDriverProfile(ctx: AuthCtx) {
  const { userId, user, profile, company } = await requireActiveProfile(ctx);
  ...
  assertSameCompany(driver.companyId, profile.companyId);

  return { userId, user, profile, company, driver, driverId };
}

export async function requireDispatcherOrAdminProfile(ctx: AuthCtx) {
  const { userId, user, profile, company } = await requireActiveProfile(ctx);

  if (profile.role !== 'DISPATCHER' && profile.role !== 'ADMIN') {
    throw new ConvexError('Este perfil no puede usar la consola de empresa.');
  }

  return { userId, user, profile, company };
}

export function assertSameCompany(actualCompanyId: Id<'companies'>, expectedCompanyId: Id<'companies'>) {
  if (actualCompanyId !== expectedCompanyId) {
    throw new ConvexError('No tienes acceso a esta información.');
  }
}

export async function assertDriverBelongsToCompany(
  ctx: AuthCtx,
  driverId: Id<'drivers'>,
  ...
  if (!driver || driver.status !== 'ACTIVE') {
    throw new ConvexError('El conductor no está disponible.');
  }

  assertSameCompany(driver.companyId, companyId);

  return driver;
}

export async function assertTripBelongsToCompany(ctx: AuthCtx, tripId: Id<'trips'>, companyId: Id<'companies'>) {
...
export async function assertDispatcherCanAccessTrip(ctx: AuthCtx, profile: ActiveProfile, tripId: Id<'trips'>) {
  if (profile.role !== 'DISPATCHER' && profile.role !== 'ADMIN') {
    throw new ConvexError('Este perfil no puede usar la consola de empresa.');
  }

  return await assertTripBelongsToCompany(ctx, tripId, profile.companyId);
}
